## Toby note
# same as orginal mass_iso from baccolite
# except now allowing oxidation of K and M
...
ms_iso <- function(sequence,ndeamidations=0,nhydroxylations=0,verbose=F){
  ...
  if(grepl('^[A-Z]+$', sequence)){
    ...
    if(nhydroxylations>0){
      max_hyd <- stringr::str_count(sequence,"P") + str_count(sequence, "K") + str_count(sequence, "M")
      if(nhydroxylations>max_hyd){
        message(sprintf("ERROR: %d hydroxylations are not possible for sequence %s",
                        nhydroxylations,sequence))
        message(sprintf("       max number of hydroxylations is %d (count of 'P' or 'K' in sequence)",max_hyd))
