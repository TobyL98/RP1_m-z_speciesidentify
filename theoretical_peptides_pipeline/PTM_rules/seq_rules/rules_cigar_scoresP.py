...
# near the Prolines (P) that are oxidised
# looks 1,2 and 3 peptides before
# and 1, 2 and 3 petides after
# the P that is oxidised
# Uses LC-MS/MS data
# These rules can then potentially be used to predict where
# prolines are oxidised in theoretical in silico generated peptides
...
def before_O_site(df, site, target_num, num_of_targets, target_num2 = 0):
    ...
    for index, row in df.iterrows():
        column = site + "hyd_count"
        if row[column] > 0:
            #pulls out sequence and modification cigar strip
            seq = row["pep_seq_bef_aft"]
            mod = row['pep_var_mod_pos']
            mod = re.sub(r"^0.|.0$", "", mod) #cleans modification cigar strip
            index_list = []
            index = 0
            ...
            for index in index_list:
                if index + target_num + 1 < len(seq) and num_of_targets == 1:
                    target = seq[index + target_num + 1]
                    target_list.append(target)
                #if two targets will create consensus
                elif index + target_num2 + 1 < len(seq) and num_of_targets == 2:
                    target = seq[index + target_num + 1] + "X" + seq[index + 1] + seq[index + target_num2 + 1]
                    target_list.append(target)


    
    #list of aa target site is used to create a dictionary with aa as the key
...
for score in pepScore_list:
    ...
    pep_aft1_df_p = before_O_site(df, site = "P", target_num = +1, num_of_targets = 1)
    ...
    pep_aft2_df_p = before_O_site(df, site = "P", target_num = +2, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_aft2_df_dict[score_heading] = pep_aft2_df_p

    pep_aft3_df_p = before_O_site(df, site = "P", target_num = +3, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_aft3_df_dict[score_heading] = pep_aft3_df_p

    pep_bef1_df_p = before_O_site(df, site = "P", target_num = -1, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_bef1_df_dict[score_heading] = pep_bef1_df_p

    pep_bef2_df_p = before_O_site(df, site = "P", target_num = -2, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_bef2_df_dict[score_heading] = pep_bef2_df_p

    pep_bef3_df_p = before_O_site(df, site = "P", target_num = -3, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_bef3_df_dict[score_heading] = pep_bef3_df_p
    
    #same but this is used to find consensus, so has two target positions
    pep_cons_df_p = before_O_site(df, site = "P", target_num = -2, target_num2 = 1, num_of_targets = 2)
    score_heading = "pep_score = {}".format(score)
...
with pd.ExcelWriter("Proline_rules.xlsx") as writer:
    pep_aft1_df_all.to_excel(writer, sheet_name = "1AfterP")
    pep_aft2_df_all.to_excel(writer, sheet_name = "2AfterP")
    pep_aft3_df_all.to_excel(writer, sheet_name = "3AfterP")
    pep_bef1_df_all.to_excel(writer, sheet_name = "1BeforeP")
    pep_bef2_df_all.to_excel(writer, sheet_name = "2BeforeP")
    pep_bef3_df_all.to_excel(writer, sheet_name = "3BeforeP")
    pep_cons_df_all.to_excel(writer, sheet_name = "Consensus")
