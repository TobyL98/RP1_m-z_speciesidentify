...
# near the lysines (K) that are oxidised
# looks 1,2 and 3 peptides before
# and 1, 2 and 3 petides after
# the K that is oxidised
# Uses LC-MS/MS data
# These rules can then potentially be used to predict where
# lysine are oxidised in theoretical in silico generated peptides
    ...
    for index, row in df.iterrows():
        ...
        if row[column] > 0:
            ...
            for index in index_list:
                ...
                elif index + target_num2 < len(seq) and num_of_targets == 2:
                    target = seq[index + target_num] + "X" + site + seq[index + target_num2]
                    target_list.append(target)


    
    #list of aa before site is used to create a dictionary with aa as the key
...
for score in pepScore_list:
    ...
    pep_aft1_df_p = before_O_site(df, site = "K", target_num = +1, num_of_targets = 1)
    ...
    pep_aft2_df_p = before_O_site(df, site = "K", target_num = +2, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_aft2_df_dict[score_heading] = pep_aft2_df_p

    pep_aft3_df_p = before_O_site(df, site = "K", target_num = +3, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_aft3_df_dict[score_heading] = pep_aft3_df_p

    pep_bef1_df_p = before_O_site(df, site = "K", target_num = -1, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_bef1_df_dict[score_heading] = pep_bef1_df_p

    pep_bef2_df_p = before_O_site(df, site = "K", target_num = -2, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_bef2_df_dict[score_heading] = pep_bef2_df_p

    pep_bef3_df_p = before_O_site(df, site = "K", target_num = -3, num_of_targets = 1)
    score_heading = "pep_score = {}".format(score)
    pep_bef3_df_dict[score_heading] = pep_bef3_df_p
    
    #same but this is used to find consensus, so has two target positions
    pep_cons_df_p = before_O_site(df, site = "K", target_num = -2, target_num2 = 1, num_of_targets = 2)
    score_heading = "pep_score = {}".format(score)
...
with pd.ExcelWriter("Lysine_rules.xlsx") as writer:
    pep_aft1_df_all.to_excel(writer, sheet_name = "1AfterK")
    pep_aft2_df_all.to_excel(writer, sheet_name = "2AfterK")
    pep_aft3_df_all.to_excel(writer, sheet_name = "3AfterK")
    pep_bef1_df_all.to_excel(writer, sheet_name = "1BeforeK")
    pep_bef2_df_all.to_excel(writer, sheet_name = "2BeforeK")
    pep_bef3_df_all.to_excel(writer, sheet_name = "3BeforeK")
