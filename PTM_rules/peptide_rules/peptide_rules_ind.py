##############################
# Code - peptide_rules_ind.py
##############################

# Code does the same as peptide_rules.py
# but input is just single organism
# LCMSMS data
...
## CHANGE THIS LINK IF USING DIFFERENT ORGANISM LCMSMS DATA OR DIFFERENT FOLDER STRUCTURE
df = pd.read_csv("C:/Users/tobyl/OneDrive - The University of Manchester/Bioinformatics Masters/Research project 1/Git_repositories/RP1_m-z_speciesidentify/PTM_rules/LCMSMS/LCMSMS_BrownRat.csv", sep = ",")

#drop all empty rows
df.dropna(how="all", inplace=True)

#change start and end columns to int
df = df.astype({"pep_start": int, "pep_end": int})
print(df.dtypes)
