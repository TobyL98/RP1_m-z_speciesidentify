...
def cross_corr(theor_peaks, act_peaks):
    ...
    if len(act_peaks) != 0:
        ...
        return(corr_diff, max_corr, max_lag)
    else:
...
def compare_corr(theor_peaks, act_peaks):
    ...
    for x in peak_list:
        ...
        corr_diff, corr_max, lag_max = cross_corr(filter_theor_peaks, filter_act_peaks)
        ...
        if corr_diff is not None:
            ...
            lag_max_list.append(lag_max)
  

    # finds the mean for corr_diff
    corr_diff_array = np.concatenate(corr_diff_list)
    corr_diff_mean = np.mean(corr_diff_array)
    corr_max_mean = sum(corr_max_list) / len(corr_max_list)
    lag_max_mean = sum(lag_max_list) / len(lag_max_list)
    return(corr_diff_mean, corr_max_mean, lag_max_mean)
...
for csv in csv_files:
    ...
    if csv_name in species_file_list:
        ...
        corr_diff, corr_max, lag_max = compare_corr(theor_peaks_df, act_peaks_df)
        csv_names = csv_name.split("_")
        species_name = csv_names[0] + " " + csv_names[1]
        cross_corr_dict[species_name] = corr_diff
        cross_max_dict[species_name] = corr_max
        cross_lag_dict[species_name] = lag_max

# converts that dictionary into a dataframe
cross_corr_df = pd.DataFrame.from_dict(cross_corr_dict, orient = "index", columns = ["Corr_diff"])
cross_max_df = pd.DataFrame.from_dict(cross_max_dict, orient = "index", columns = ["Corr_max"])
cross_lag_df = pd.DataFrame.from_dict(cross_lag_dict, orient = "index", columns = ["Corr_lag"])
...
final_df = final_df.merge(cross_max_df, how="inner", left_on= "SPECIES", right_index= True)
final_df = final_df.merge(cross_lag_df, how="inner", left_on= "SPECIES", right_index= True)
...
final_df = final_df.sort_values(by= ["Corr_max"], ascending= False)
